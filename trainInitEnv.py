...
parser.add_argument('--renderedEnv', type=float, default=1.0, help = 'the weight of training network for renderedImg')
# Refine the network
...
# [kavidaya] Load this network here...
# Global illumination
# globIllu1to2 = nn.DataParallel(models.globalIllumination(), device_ids = opt.deviceIds)
# globIllu2to3 = nn.DataParallel(models.globalIllumination(), device_ids = opt.deviceIds)
...
while epoch < opt.nepoch:
    ...
    for i, dataBatch in enumerate(brdfLoader):
        ...
        globalIllu1Gt = renderLayer.forward(albedoBatch, normalBatch, roughBatch, depthBatch, segBatch).detach()

        renderedEnvGt = renderLayer.forwardEnv(albedoBatch, normalBatch, roughBatch, SHBatch, segBatch)
        
        # [kavidaya] Detach is added here??? Do we seriously not need to backpropogate beyound one level???
        # inputGlob2 = torch.cat([globalIllu1Gt, albedoBatch, normalBatch, roughBatch, depthBatch, segBatch], dim=1)
        # globalIllu2Gt = globIllu1to2(inputGlob2).detach()
        # inputGlob3 = torch.cat([globalIllu2, albedoBatch, normalBatch, roughBatch, depthBatch, segBatch], dim=1)
        # globalIllu3Gt = globIllu2to3(inputGlob3).detach()
        # globalIllu2Gt = 0.5*(globalIllu2 + 1) * segBatch.expand_as(imBatch )
        # globalIllu3Gt = 0.5*(globalIllu3 + 1) * segBatch.expand_as(imBatch )
        ...
        lossQid = lossMAE(idSynthetic, inputInit[:,0:3,:,:]) + lossMAE(idReal, inputRealInit[:,0:3,:,:])# L1 norm between synthetic images + L1 norm between real images 
        ...
        lossQz = 0.5*lossMSE(predLabels, labels)
        ...
        # Loss 4 : 
        lossQcyc = lossMAE(decoderXInit(*encoderYInit(torch.cat((decoderYInit(x1, x2, x3, x4, x5, xSynth), inputInit[:,3:4,:,:]),dim=1))),inputInit[:,0:3,:,:]) + lossMAE(decoderYInit(*encoderXInit(torch.cat((decoderXInit(y1, y2, y3, y4, y5, xReal),inputRealInit[:,3:4,:,:]),dim=1))), inputRealInit[:,0:3,:,:])
        ...
        renderedEnvErrSum = sum(renderedEnvErrs)
        ...
        renderedEnvErrSumTrc = sum(renderedEnvErrsTrc)
