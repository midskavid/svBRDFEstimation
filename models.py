...
class encoder(nn.Module ):
    def __init__(self ):
        ...
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=32, kernel_size=6, stride=2, padding=2, bias=False)
        ...
        self.pad6 = nn.ZeroPad2d((1, 1, 2, 2) )
        self.conv6 = nn.Conv2d(in_channels=256, out_channels=512, kernel_size=4, stride=2, bias=False)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.bn6(self.conv6( pad6(x5) )), True )
...
class decoder(nn.Module ):
    def __init__(self, mode=0):
        ...
        self.dconv0 = nn.ConvTranspose2d(in_channels=512, out_channels=256, kernel_size=4, stride=2, padding=(2, 1), output_padding=(1, 0), bias=False)
...
class globalIllumination(nn.Module):
    ...
    def forward(self, x):
        ...
        x_out  = F.tanh( self.convFinal(x_d5) )
        return x_out
...
class refineEncoder_point(nn.Module):
    def __init__(self):
        super(refineEncoder_point, self).__init__()
        # Encoder: segmentation: 1
        # albedo: 3, normal: 3 roughness: 1, depth: 1, two input image 4, total chnum = 15
        self.conv1 = nn.Conv2d(in_channels=12, out_channels=64, kernel_size=6, stride=2, padding=2, bias=False)
...
class refineEncoder_error_point(nn.Module):
    def __init__(self):
        super(refineEncoder_error_point, self).__init__()
...
class refineEncoder_error(nn.Module):
    def __init__(self):
        super(refineEncoder_error, self).__init__()
        # Encoder: the input will be error: 3 segmentation: 1
        # albedo: 3, normal: 3 roughness: 1, depth: 1, two input image 6, total chnum = 18
...
class refineEncoder_grad(nn.Module):
    def __init__(self):
        super(refineEncoder_grad, self).__init__()
        # Encoder: the input will be segmentation: 1
        # albedo: 3, normal: 3 roughness: 1, depth: 1, two input image 6, total chnum = 18
        self.conv1 = nn.Conv2d(in_channels=23, out_channels=64, kernel_size=6, stride=2, padding=2, bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.conv2 = nn.Conv2d(in_channels=64, out_channels=128, kernel_size=6, stride=2, padding=2, bias=False)
        self.bn2 = nn.BatchNorm2d(128)
        self.res1 = residualBlock(128)
...
class refineDecoder(nn.Module):
    ...
    def forward(self, x1, x3):
        ...
        x_orig = F.tanh(self.convFinal(x_d1) )
        if self.mode == 0:
            x_out = x_orig
        elif self.mode == 1:
            norm = torch.sqrt(torch.sum(x_orig * x_orig, dim=1).unsqueeze(1) ).expand_as(x_orig);
...
class refineEnvDecoder_grad(nn.Module):
    def __init__(self):
        super(refineEnvDecoder_grad, self).__init__()
        self.numCoef = 9
        self.conv1 = nn.Conv2d(in_channels = 128, out_channels=256, kernel_size=4, stride=2, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(256)
        self.conv2 = nn.Conv2d(in_channels = 256, out_channels=256, kernel_size = 4, stride=2, padding=1, bias=False)
    ...
    def forward(self, x, pred, grad):
        ...
        x = self.pool(x)
        x = x.view( x.size(0), -1)
        grad = grad.view( grad.size(0), 3*self.numCoef)
        pred = pred.view( pred.size(0), 3*self.numCoef)
        pred_grad = self.projection(torch.cat([pred, grad], dim=1) )
        x = torch.cat([pred_grad, x], dim=1)
        x = F.tanh(self.regression(x) )
        x = x.view( x.size(0), 3, self.numCoef)
        return x
